...
## 下一步（R-04 完成缺口；2026-09-11 立项）

- [ ] 先定路线：外部数据补充 vs 结论收缩 vs 现有数据深挖（三选一，需用户拍板；外部数据需先批后取）
- [ ] marker-proxy 扩样本验证              a29 单患者结论待复核；需更多患者，可与 TSB 追查并行
    - [ ] TSB fold-4 rank3 的 0.544 追查（孤立信号，真信号还是噪声）
    - [ ] 组成与状态分量＋不确定性量化（R-04 完成判据要求， currently 缺）
- [ ] 未命名场的可重复性检验                目前只验过有锚点的场；无锚点场尚未检验
- [ ] 独立 lineage 复现                    暂缓中；需新训练＝需 GPU 审批
...
- K=2 Stage A：已完成并入主线（D-106，2026-09-10；L-001 关闭）。
- 独立 lineage 复现：暂缓（无新训练，validation 拟合不存在）。
