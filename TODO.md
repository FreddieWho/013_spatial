...
- [x] ① P-mhc2 TLS指向验证完成（D-134：细窗963配对B轴显著更好p=3.3e-08，退回场活性描述；L-009关闭）
- [ ] ② T2P2 主 surrogate（LEADS L-010；Lane A永久NOT_CALIBRATED亦可接受）
- [ ] ③ TLS–肿瘤走廊（等距角度对照，一会做）  须在残差分上做；设计见 docs/field_geometry_scanner.md 特化段
    - [ ] a 每个 TLS 焦点连最近 TUM，走廊扇区 vs 同距离 90° 对照
    - [ ] b 分数=Q+C 残差（禁止 raw log1p）；6 程序 + 抗原呈递家族先跑
...
- 2026-09-21：径向 spline 关闭（D-139）。新开暂缓待办③走廊检验；泛用眼走残差几何字典（L-012），不先上 CNN。
## 变更记录
...
- 2026-09-19：L-009完成（D-134）。mhc2超B轴不稳定，T5B线索修订为不成立；T4活性证据不受影响。剩余②T2P2待定。
- 2026-09-21：spline 三路线关闭径向形态（D-139）。新增待办③ TLS–肿瘤走廊（一会做）。泛用几何扫描仪设计落盘 docs/field_geometry_scanner.md；CNN 降为字典不够用时的升级（LEADS L-012）。
