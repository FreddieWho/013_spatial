...
## I-019｜R-04 合成科学校准已通过，但真实数据 K 与患者级场仍未运行
- **状态：** `OPEN`
- **发现日期：** 2026-08-13
- **影响节点：** R-04 正式候选发现及 H-01/H-03/H-05。
- **当前事实：** v10 合成门禁在同一数据、同一评价基因和 data-seed 独立统计下，能在无场、单场、双独立场和共线双源中选择对应 `K_eff`；CPU 资源硬阻塞已解除，fit 入口也已接通模型隔离断点目录，但真实 47-section 数据尚未完成患者/组织块留出的 K=0 向上粗筛、五次 restart、跨 restart 子空间稳定性、组成 posterior 或独立 lineage 复现。
- **当前影响：** `PASS_SCIENTIFIC_CALIBRATION` 不能写成发现了患者空间场，合成 K=2 不能写成真实最终 K；R-04 roadmap 节点仍未完成。
- **下一判定点：** 先实现并运行真实数据 K=0 向上的患者/组织块留出粗筛；只有非边界 K 明确后才运行五次 restart，并报告 K=0 竞争、搜索边界、子空间稳定性和 held-out 增量。
- **硬阻塞条件：** 若真实 K 持续卡在搜索上界、跨 restart 不稳定或不优于 K=0/null，状态恢复为 `K_NOT_IDENTIFIABLE`，不得进入场命名和后续重建。
