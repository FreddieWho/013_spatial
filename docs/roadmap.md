...
**2026-09-14 场发现策略修正（D-112）：残余降级为评分维度，field registry 立项。** TLS 阳性对照（`infra/r04/tls_positive_control_20260912.json`，13,726 标注点/170 阳性/4 片/3 患者，exploratory）：经典 TLS 基因（CXCL13/MS4A1/CD3/CD19/CCL19/CCL21/LTB）全部不在 4000 基因 panel；PTPRC AUC 0.78、组合 0.93 样本内可测；浆模块 0bd3 0.81 vs a29 0.34–0.38（符号翻转）；最强信号恰为组成型（白细胞富集＋非上皮）。用户指出：组成当然是场的一部分，以"组成之外残余"作为参赛资格会错杀本质即组成的真实结构；双方共识：搜索完全放开（任何场都可被发现与登记），升级为 claim 必须跨患者/队列复现＋过相应 null＋量化不确定性（守门只问方法不问内容，null 实现允许因场制宜）。残余降级为评分维度之一，不单独否决任何场；新增 field registry（scored list，含复现/null/残余/效应量/不确定性/证据等级）为持久资产，账本永不关闭，停止规则内建；预测器工具保留并加双头输出（组成解释量＋残余量）与输出自带证据等级两条修正。R-04 阴性关闭（D-110）不改写不回溯——本条改变后续场发现与登记方式，不下调其结论。下一步：registry schema＋回填现有候选；R-11 smoke 与数据申请维持 hold。
...
- H-01：R-04、R-05、R-07、R-16。
- H-02：R-06、R-07、R-14。
- H-03：R-04、R-05、R-16。
- H-04：R-08、R-09、R-14。
- H-05：R-04、R-08、R-10、R-14、R-16。
- H-06：R-11。
- H-07：R-12、R-13。

没有未被非基础设施节点引用的 plan.md 假设。

## R-16｜空间 pattern 普查与距离函数发现引擎（D-112/D-113 框架）`[探索可并行]`

**2026-09-19 R-16 恢复包关门（T0–T6完成，D-133；分支 recovery-20260918 待合并）：** T1符号修复857→772组NEW=0维持；T2-small同算子闭环（关联p记NOT_CALIBRATED）；T3 191→6冻结程序；T4独立复现+增量（M1全员成立，M2仅mhc2/stress）；T5场预测成立、mhc2超B轴成TLS最强线索。H-01/H-03支持，H-05部分支持，H-02/H-06/H-07未测试。旧D-118/D-126数字保留、外推维持METHOD_LIMITED（T2P2/深度确证未做）。R-04仍关闭。

**2026-09-18 R-16 恢复包开工（T0 完成，分支 recovery-20260918）：** 外部审计定位 Lane A/B 与 PC 工厂的方法实现问题（A02/A05/A06/A09/A10/A13/A15/A18；ISSUES I-022/I-023/I-024/I-025）。D-118 NEW=0、D-126 全线阴性的历史数字保留，其普遍否证外推标记为 `METHOD_LIMITED_PENDING_REANALYSIS`；D-127 的 191 保留为候选（分层评价替代"排除亚型/状态"）。T0 查明：源 X=int64 原始 counts，cache 原样拷贝，r16 路径无深度校正系分析选择（A14 已定位上游，非数据缺失）。R-04 关闭不受影响。下一步 T1（PC 符号修复）→T2（Lane A 同算子）→T3（191→≤10 程序）→T4（ST-CRC/USZ 独立复现＋增量）→T5/T6。服务假设：H-01/H-03（T1–T4 直接检验）、H-05（T4 跨队列）。


1. **要做什么：** 在 frozen panel cache 上分步执行。第一步（组成 pattern 普查）：每张训练片做表达＋空间图聚类（多档分辨率），产出 cluster mask，同队列内按 marker signature 匹配为跨片 pattern 组，跨分辨率稳定性入分，普遍性**按病人计数**。第二步（距离函数筛选）：每个登记 pattern 自动生成距离尺（几何选择与分辨率地板写入登记行）；Lane A 在全基因×全尺上筛距离函数关系（预选函数族：等调单调＋限自由度样条；壳层以单峰约束单列）；Lane B 回归掉全部距离协变量后筛残余空间结构。所有连续信号检验使用空间保持 null（场相对 mask 旋转/平移、变异函数匹配 GP 模拟、块置换），禁用 naive 值置换与标签置换。全部产出按 D-112 统一证据标准进 field registry。
2. **服务的假设：** H-01（Lane B 为其 D-112 后操作化）、H-03（残余维度列）、H-05（跨患者一致性）。
3. **完成判据：** 产出第一版 field registry：Lane A 复发距离函数表＋Lane B 残余结构表，每行含复现（病人数）、null、残余维度、效应量、不确定性、证据等级；任何行不得以单患者证据升级为 claim。
4. **结果如何改变信心：** Lane A 出现 ≥2 病人同向复现的距离函数 → H-01/H-03 信心上调并按残余维度定级；Lane B 在 Lane A 充分后无可复现残余 → H-01 未命名场分支按否证逻辑收缩；普查只产组成型 pattern 也是合法资产（D-112）。

**2026-09-16 GraphST GPU 臂全量完成（D-121/D-122）：** V100 上逐片训练 47 片×600 epoch（merged 稠密矩阵约 100GB 不可行，改逐片；seurat_v3 top-3000 原生流程；Leiden 代 mclust）。1 片 HVG loess 退化走 seurat-fallback（已披露）。995 片内 cluster → 平均链接匹配成 356 组，32 组复现（背景持家 20+病人；平滑肌/基质/浆/杯状/肠上皮/Wnt/TLS 样/MHC 等，多为 2–3 病人）。评分前拦截两处静默腐败：跨片池化捏造（D-123，已加回归测试）、定宽字符串截断组 id。registry 共 1657 行。GPU 任务结束即退租。

**2026-09-15 联合嵌入轨（D-119，两臂全量完成）：** HTAN 内部 47 片/113k 点联合：Arm A（ComBat 按片矫正＋PCA＋Leiden）与 Arm B（BANKSY 式邻域增强 λ=0.5 后同流程）各出 99/117 候选；split-half 病人对半独立重跑（半内重做 ComBat），216/216 双半匹配≥0.75（均值谱余弦门偏松，均值中位 0.93，如实记录）。67 个≥2 病人候选与 Tier-2 的 marker 交集<0.3（含杯状细胞 MUC5B/TFF1 17 病人、Paneth 样 DEFA5/6 10 病人、Wnt 程序、MHC-II 等）——联合轨捞回了被 Tier-2 毛发团埋掉的程序。registry 共 1301 行。R 专用臂（Seurat SCT/Harmony/PRECAST）进行中。

**2026-09-15 Tier-1 v2 尺子工厂（D-118，可信阴性）：** 邻域增强×λ（0/0.2/0.5/0.8）→逐片逐λ PCA top-10→LISA tile（999 次置换＋BH）→新旧对照→loading 匹配。1880 候选→857 组，仅 2 组复现且均为 KNOWN（持家/Epi 29 病人；基质 3 病人），NEW 复现轴=0；跨病人 loading 相似度中位 0.306 且无阈值膝点；半径复核 0.975 通过。registry 升 v1.1（1085 行）。含义：病人特异性主导，未命名共享程序在本队列 loading 形式下不可见。

**2026-09-15 独立复现（D-116）：** 同一套 7 轴不混发现集，打到 ST-CRC（CRC，7/7 相干，CXCL13 峰≤+1 为 6/7）与 USZ（肾+肺，8/8 相干，CXCL13 7/8；TLS~B 中位 AUC 0.71，肾高肺杂）。发现 registry 不改写。Lane A 仍待空间保持 null（I-021）。

**2026-09-14 节点创建（D-113）：** 用户质疑"R-04 空转后只发现 4 个信号"，核实确认仓库从未有 pattern 挖掘代码、结构清单继承自文献注释——发现扫描从未被执行。用户立原理：组成 pattern 是地基，目标是与组成 pattern 距离构成函数关系的连续信号；开关变量检验对连续信号不适用。定型：距离函数框架＋双车道＋分步 DAG＋空间保持 null 换代。执行序列①普查→②距离变换→③Lane A→④Lane B→⑤registry。待办：一页设计方案（函数族/null 类型/分辨率档位/阈值/registry schema）交用户过目后开跑。
