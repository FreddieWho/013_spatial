# R-16 设计方案 v2.0（2026-09-14，D-115 两层结构版）

依据 D-112/D-113/D-114/D-115。本文档冻结参数；改动需追加 decision。
...
| 参数 | 取值 |
|---|---|
| 轴 | T(99)/B(45)/Mye(57)/ILC(12)/Epi(125)/Stromal(55)/Plasma(6) 基因（D-105 marker 库 ∩ HVG-10k＋浆模块），可扩 |
...
已完成：materialize（47 片/10k 基因）、Tier-1（CPU 分钟级）、Tier-2（CPU ~10 分钟）、registry v1。产出物全部 `EXPLORATORY_*_NOT_CLAIM`。

## 9. 已知限制（随结果一起交付）

单 lineage；坐标跳数近似；毛发球吞弱特异性 pattern（L-006 marker-overlap 候选解）；ILC 轴弱（12 基因，相干 27/30）；轴集合覆盖受 marker 库限制（D-105 失效条件沿用）；mask 视为固定（不确定性传播留敏感性检查）。
